...
func _physics_process(delta: float) -> void:
	...
	if turn and name == str(Steamworks.STEAM_ID) and Input.is_action_just_pressed("touch_main"):
		...
		send_vector_packet(new_vector, name)
		turn = false
		in_motion = true
	# Handle collisions
	...
	if collision_info:
		...
		if collider.is_in_group("balls"):
			send_vector_packet(ball_vector/2, collider.name)
...
func send_vector_packet(new_vector, ball_id):
	var DATA = PoolByteArray()
	DATA.append(Lobby.Packet.VECTOR_UPDATE)
	DATA.append_array(var2bytes({"vector":new_vector, "ball":ball_id}))
